...
## Queries
- Don't add `SELECT *` to existing code that names columns.
- Don't add new indexes speculatively — call them out as a suggestion instead.
- Don't change `JOIN` order or types (`INNER` ↔ `LEFT`) unless that's the bug.
- For aggregations, match existing alias style.

## Performance
- Don't add `EXPLAIN ANALYZE` output to context — summarize it.
- Don't suggest denormalization or schema redesign as a side-effect of a query fix.

## Never
- Don't use vendor-specific features (Postgres `JSONB`, MySQL `ON DUPLICATE KEY`) if the codebase targets multiple DBs.
- Don't add stored procedures / triggers unless the codebase already uses them.
